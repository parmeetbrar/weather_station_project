...
6. The GUI is shown below. The indoor environment controls are in the "Indoor" section, the Heater, AC, and Auto Lights indicators should go from red to green when a function is actiavted. The Auto and Energy saving mode buttons will go from red to green when clicked. The activation of these environmental control divices is simulated with LED lights for this project. The refresh rate can be adjusted with the up and down arrows beside the refresh rate.
